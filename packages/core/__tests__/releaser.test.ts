...
describe("updatePackageVersion", () => {
  let tempDir: string;
  beforeEach(async () => { tempDir = await mkdtemp(join(tmpdir(), "rs-releaser-")); });
  ...
  it("updates version in package.json", async () => {
    ...
    await writeFile(join(pkgDir, "package.json"), JSON.stringify({ name: "@myapp/core", version: "1.0.0" }, null, 2) + "\n");
  ...
  it("preserves other fields", async () => {
    await writeFile(join(tempDir, "package.json"), JSON.stringify({ name: "@myapp/core", version: "1.0.0", description: "Core lib" }, null, 2) + "\n");
...
describe("updateWorkspaceDeps", () => {
  ...
  it("updates workspace dependency versions", async () => {
    ...
    await writeFile(join(tempDir, "packages/cli/package.json"),
      JSON.stringify({ name: "@myapp/cli", version: "1.0.0", dependencies: { "@myapp/core": "workspace:*" } }, null, 2) + "\n");
  ...
  it("updates peerDependencies too", async () => {
    ...
    await writeFile(join(tempDir, "packages/plugin/package.json"),
      JSON.stringify({ name: "@myapp/plugin", version: "1.0.0", peerDependencies: { "@myapp/core": "^1.0.0" } }, null, 2) + "\n");
    const versionMap = new Map([["@myapp/core", "2.0.0"]]);
    await updateWorkspaceDeps(join(tempDir, "packages/plugin"), versionMap);
    const content = JSON.parse(await readFile(join(tempDir, "packages/plugin/package.json"), "utf-8"));
